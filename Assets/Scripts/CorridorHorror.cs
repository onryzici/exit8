...
public class CorridorHorror:MonoBehaviour {
 ...
 public bool HasVisibleChange=>watcher.activeSelf||water.activeSelf||emergency.gameObject.activeSelf||Mode==4||(encounters&&encounters.HasVisibleChange);
 AudioSource source;AudioClip knock,breath,rush;Vector3 doorPosition;Quaternion doorRotation;ColorGrading grade;float exposure,age,nextSound,floodTravel,warningAt;bool initialized,floodStarted;Material waterMaterial,entryMaterial;ParticleSystem floodMist;HorrorEncounters encounters;
 public HorrorEncounters Encounters=>encounters;
 void Init(){if(initialized)return;initialized=true;source=new GameObject("Anomaly spatial audio").AddComponent<AudioSource>();source.transform.SetParent(transform);source.playOnAwake=false;source.spatialBlend=1;source.minDistance=1.5f;source.maxDistance=24;source.rolloffMode=AudioRolloffMode.Logarithmic;source.dopplerLevel=0;source.reverbZoneMix=.75f;knock=Sound(false);breath=Sound(true);var data=new float[22050*4];var rng=new System.Random(85);float noise=0;for(int i=0;i<data.Length;i++){noise=Mathf.Lerp(noise,(float)rng.NextDouble()*2-1,.13f);data[i]=noise*(.45f+.1f*Mathf.Sin(i/22050f*5));}rush=AudioClip.Create("Water rushing through passage",data.Length,1,22050,false);rush.SetData(data,0);waterMaterial=water.GetComponent<Renderer>().material;entryMaterial=entryWater.GetComponent<Renderer>().material;ConfigureFlood();encounters=gameObject.AddComponent<HorrorEncounters>();encounters.Initialize(this);}
 void ConfigureFlood(){
  foreach(var material in new[]{waterMaterial,entryMaterial})material.SetColor("_Color",new Color(.045f,.095f,.105f,1));
  loop.player.view.depthTextureMode|=DepthTextureMode.Depth;
  if(!floodSpray)return;
  floodMist=Instantiate(floodSpray,floodSpray.transform.parent);floodMist.name="Flood / fine suspended mist";
  var mistMain=floodMist.main;mistMain.startSize=new ParticleSystem.MinMaxCurve(.16f,.38f);mistMain.startLifetime=new ParticleSystem.MinMaxCurve(.35f,.7f);mistMain.gravityModifier=.08f;
  var mistEmission=floodMist.emission;mistEmission.rateOverTime=65;
  var fade=floodMist.colorOverLifetime;var gradient=new Gradient();gradient.SetKeys(new[]{new GradientColorKey(new Color(.72f,.79f,.81f),0),new GradientColorKey(new Color(.8f,.85f,.86f),1)},new[]{new GradientAlphaKey(0,0),new GradientAlphaKey(.09f,.15f),new GradientAlphaKey(0,1)});fade.color=gradient;
  var main=floodSpray.main;main.startSize=new ParticleSystem.MinMaxCurve(.012f,.052f);main.startLifetime=new ParticleSystem.MinMaxCurve(.28f,.65f);main.gravityModifier=1.35f;main.maxParticles=700;
  var emission=floodSpray.emission;emission.rateOverTime=420;
  var velocity=floodSpray.velocityOverLifetime;velocity.y=new ParticleSystem.MinMaxCurve(1.1f,3.2f);velocity.z=new ParticleSystem.MinMaxCurve(-3.8f,-1.8f);
  var renderer=floodSpray.GetComponent<ParticleSystemRenderer>();renderer.renderMode=ParticleSystemRenderMode.Stretch;renderer.lengthScale=1.6f;renderer.velocityScale=.055f;
  floodMist.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);
 }
 ...
 void Update(){if(Mode<0)return;float dt=Time.deltaTime;age+=dt;var p=loop.player.transform.position;bool inside=Mathf.Abs(p.x)<2.4f&&p.z>2&&p.z<25;
  ...
  if(encounters)encounters.Tick(dt);
  if(Mode<0)return;
  if(Mode==10){
   ...
   if(warningAt>=0&&age-warningAt>=floodWarningSeconds){floodStarted=true;floodTravel=(age-warningAt-floodWarningSeconds)*floodSpeed;entryWater.SetActive(true);float entryLength=Mathf.Min(6.4f,.4f+floodTravel);entryWater.transform.SetPositionAndRotation(new Vector3(6.2f-entryLength*.5f,.01f,26.3f),Quaternion.Euler(0,90,0));entryWater.transform.localScale=new Vector3(3.34f,1,entryLength);entryMaterial.SetFloat("_FrontZ",6.2f-entryLength);
    if(floodTravel>3.8f){water.SetActive(true);float length=Mathf.Min(27,.4f+floodTravel-3.8f);float front=26.3f-length;water.transform.localScale=new Vector3(4.46f,1,length);water.transform.position=new Vector3(0,.01f,26.3f-length*.5f);waterMaterial.SetFloat("_FrontZ",front);source.transform.position=new Vector3(0,1,front+.5f);if(floodSpray){floodSpray.transform.position=new Vector3(0,1.3f,front+1.8f);if(!floodSpray.isPlaying)floodSpray.Play();if(floodMist){floodMist.transform.position=floodSpray.transform.position;if(!floodMist.isPlaying)floodMist.Play();}}
     if(loop.commuter&&loop.commuter.Walking&&loop.commuter.transform.position.z>front+1.8f)loop.commuter.HideInFlood();
 ...
 public void Clear(){if(encounters)encounters.Clear();if(loop&&loop.commuter)loop.commuter.RestoreAfterFlood();if(Mode==4&&loop&&loop.door){loop.door.transform.localPosition=doorPosition;loop.door.transform.localRotation=doorRotation;}if(grade){grade.postExposure.value=exposure;grade=null;}if(source){source.Stop();source.loop=false;source.volume=1;}if(watcher)watcher.SetActive(false);if(water)water.SetActive(false);if(entryWater)entryWater.SetActive(false);if(floodSpray)floodSpray.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);if(floodMist)floodMist.Stop(true,ParticleSystemStopBehavior.StopEmittingAndClear);if(emergency)emergency.gameObject.SetActive(false);floodStarted=false;Mode=-1;}
 void OnDestroy(){Clear();if(floodMist)Destroy(floodMist.gameObject);if(knock)Destroy(knock);if(breath)Destroy(breath);if(rush)Destroy(rush);if(waterMaterial)Destroy(waterMaterial);if(entryMaterial)Destroy(entryMaterial);}
}
