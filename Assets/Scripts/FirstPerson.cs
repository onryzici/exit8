...
public class FirstPerson : MonoBehaviour {
    ...
    float stamina=1,recoveryDelay;bool winded;
    public bool HorrorLocked {get;set;}
    public bool IsSprinting=>running;
    public float Exhaustion=>1-stamina;
    public void ResetBreath(){stamina=1;recoveryDelay=0;winded=false;running=false;HorrorLocked=false;}
    ...
    void Update(){
        if(HorrorLocked){running=false;return;}
        ...
        bool wantsSprint=Input.GetKey(KeyCode.LeftShift)&&direction.sqrMagnitude>.01f&&!crouch;
        bool sprint=UpdateStamina(Time.deltaTime,wantsSprint);
        ...
        vertical-=18*Time.deltaTime; motor.Move((direction*(crouch?1.5f:sprint?4.3f:2.65f)+Vector3.up*vertical)*Time.deltaTime);
    ...
    public bool UpdateStamina(float deltaTime,bool wantsSprint){
        bool sprint=wantsSprint&&!winded&&stamina>.025f;
        if(sprint){stamina=Mathf.Max(0,stamina-deltaTime/3.8f);recoveryDelay=.85f;if(stamina<.025f)winded=true;}
        else{recoveryDelay-=deltaTime;if(recoveryDelay<=0)stamina=Mathf.Min(1,stamina+deltaTime/6);if(stamina>.35f)winded=false;}
        return sprint;
    }
    void OnGUI(){
        if(stamina<.98f){
            GUI.color=new Color(.04f,.04f,.04f,.6f);GUI.DrawTexture(new Rect(28,Screen.height-57,116,3),Texture2D.whiteTexture);
            GUI.color=winded?new Color(.65f,.30f,.25f,.8f):new Color(.7f,.72f,.69f,.65f);
            GUI.DrawTexture(new Rect(28,Screen.height-57,116*stamina,3),Texture2D.whiteTexture);
        }
